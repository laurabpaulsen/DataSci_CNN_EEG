...
from utils.cnn import prep_dataloaders, prep_model, CNN
...
def main():
    ...
    gafs, labels = np.load(gaf_path / f"{args.sub}_gafs.npy"), np.load(gaf_path / f"{args.sub}_labels.npy")
    ...
    if not sub_mdl_path.exists():
        sub_mdl_path.mkdir(parents=True)
